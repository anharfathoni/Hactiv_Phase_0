...
function nextTargetArea(code) {
    var huruf = "WFXVG"
    var angka = "24678"
    var spell = ["two", "four", "six", "seven", "eight"]
    var sumspell = 0
    var remainSpell = 0
    var result = 0
    ...
    for (var i = 0; i < code.length; i++) {
        for (var j = 0; j < huruf.length; j++) {
            if (code[i] === huruf[j]) {
                result += Number(angka[j])
                sumspell += spell[j].length
            }
        }
    }
    // console.log(result)
    remainSpell = code.length - sumspell
        // console.log("remain:", remainSpell)
    if (remainSpell % 5 === 0) {
        result += (remainSpell / 5) * 3
    }
...
//TAMBAHAN
console.log(nextTargetArea('TWOEIGHTTHREESIXTHREESIX')); // "District 28 is the next target!"