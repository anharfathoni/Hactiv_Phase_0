...
function getGuildMemberInfo(members) {
  ...
  var averageLevel = Math.floor(jumlah/members.length);
  ...
  for (let i = 0; i < members.length; i++) {
    if (!result[members[i].class]) {
      ...
      if (members[i].level >= averageLevel) {
        lulus = {name: members[i].name, level: members[i].level};
        result[members[i].class].push(lulus);
      }
    }
    else {
      if (members[i].level >= averageLevel) {
