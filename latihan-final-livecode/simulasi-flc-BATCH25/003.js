...
function faktorPrima(num) {
    ...
    for (var i = 0; i < prima.length; i++) {
        while (num % prima[i] === 0) {
            faktorPrima.push(prima[i])
            num = num / prima[i]
        }
    }
    // console.log("faktor:",faktorPrima)

    for (var i = 0; i < prima.length; i++) {
        var result = ""
        var count = 0
        for (var j = 0; j < faktorPrima.length; j++) {
            if (prima[i] === faktorPrima[j]) {
                count += 1
            }
        }
        if (count > 0) {
            result += prima[i] + " pangkat " + count
