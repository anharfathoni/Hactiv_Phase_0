...
function oddPairFirstLast(num) {
    ...
    if (strNum === "0") {
        ...
        var firstlast = strNum[0] + strNum[strNum.length - 1]
            // console.log(firstlast)
        if (strNum.length === 0) {
            return 0
        }
        if (Number(firstlast) % 2 !== 0) {
            return Number(firstlast) + Number(oddPairFirstLast(Number(strNum.slice(1, strNum.length - 1))))
        } else {
            return oddPairFirstLast(Number(strNum.slice(1, strNum.length - 1)))
        }
    } else {
        var firstlast = strNum[0] + strNum[strNum.length - 1]
        if (strNum.length === 1) {
            return Number(strNum[0] + 1)
        }
        if (Number(firstlast) % 2 !== 0) {
            return Number(firstlast) + Number(oddPairFirstLast(Number(strNum.slice(1, strNum.length - 1))))
        } else {
