...
function simpleCardsFusion(str) {
    ...
    var check = false
    for (var j = 0; j < fusionCards.length; j++) {
        if (strSPlit[0] === fusionCards[j].card1 || strSPlit[0] === fusionCards[j].card2) {
            if (strSPlit[1] === fusionCards[j].card1 || strSPlit[1] === fusionCards[j].card2) {
                // return fusionCards[j].name
                check = true
                indeks = j
                break
            }
    ...
    if (check === true) {
        return fusionCards[indeks].name
    } else {
