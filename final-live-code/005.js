...
function classReport(students) {
    ...
    for (var i = students.length - 1; i > 0; i--) {
        ...
        if (uniqClass === true) {
            kelas.unshift(students[i].class)
        }
    }
    kelas.unshift(students[0].class)
        // console.log(kelas)
    ...
    //     delete students[a]["class"]
    // }
    // console.log(students)
    for (var i = 0; i < kelas.length; i++) {
        // objSiswa[kelas[i]].pass.push(students[0])
        // objSiswa[kelas[i]].fail.push(students[1])
        for (var j = 0; j < students.length; j++) {
            if (students[j].class === kelas[i]) {
                if (students[j].score >= 80) {
                    delete students[j].class
                    objSiswa[kelas[i]].pass.push(students[j])
                } else {
                    delete students[j].class
                    objSiswa[kelas[i]].fail.push(students[j])
                }
            }
        }
    }
